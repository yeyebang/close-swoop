...
本次版本主要修复 V4 Web 扫描上线后的两个体验问题：扫描大盘没有弹出进度窗口，以及部署后浏览器/Cloudflare 可能继续加载旧前端资源。

| 模块 | 修改 | 目的 |
|------|------|------|
| `ui_server.py` | `/api/v4/market-scan` 改为启动后台线程 `start_v4_market_scan()` | 避免 V4 扫描同步请求被线上代理或浏览器超时中断 |
| `ui_server.py` | V4 扫描复用 `SERVER_STATE`，新增 `scan_kind`、`scan_result` | 前端可用统一 `/api/scan/status` 轮询 V4 扫描进度和结果 |
| `v4_strategy.py` | `create_market_scan()` 新增 `progress` 回调 | 将获取行情、风控预筛、处理进度、入池/剔除数量写入扫描弹窗日志 |
| `web/app.js` | `runMarketScan()` 改为 `runV4MarketScan()`，打开弹窗后启动 V4 异步扫描 | 恢复“扫描大盘”按钮的进度弹窗体验 |
| `web/app.js` | API 请求增加 HTTP 非 2xx 错误处理 | 红色报错能显示真实后端错误，而不是静默失败 |
| `ui_server.py` | HTML/JS/CSS 响应增加 `Cache-Control: no-store` 等响应头 | 降低线上部署后继续加载旧脚本的概率 |
| `web/index.html` | `app.js` / `styles.css` 增加版本查询参数 | 强制浏览器和 CDN 在部署后获取新版前端资源 |
...
点击“扫描大盘”
  → openScanModal()
  → POST /api/v4/market-scan
  → 后端后台线程运行 create_market_scan(progress=append_scan_log)
  → 前端轮询 /api/scan/status
  → 弹窗显示阶段、日志、完成/失败结果
```

线上部署注意：

- 部署后如果弹窗仍不出现，优先检查浏览器是否加载了新版 `/app.js?v=...`。
- Cloudflare / Nginx / 浏览器任一层缓存旧 `index.html` 都可能导致前端仍运行旧逻辑。
- 当前服务端已对 `.html`、`.js`、`.css` 返回 no-cache，但若上游 CDN 强制缓存，需要清理 CDN 缓存或用带版本参数的 URL 验证。
...
点击“扫描大盘”或旧版“开始扫描”后弹出进度弹窗，提供可视化扫描状态反馈。v4.2 起，V4「扫描大盘」也使用同一套弹窗和轮询状态，不再同步等待 `/api/v4/market-scan` 返回。
...
- `runV4MarketScan()` — 启动 V4 扫描大盘时打开弹窗，随后轮询统一扫描状态
- `runScan()` — 旧版扫描入口，保留兼容
- `pollScanStatus()` — 轮询状态，完成后自动关闭（成功延迟 1.5s）

**后端状态** (`ui_server.py`):
- `SERVER_STATE` 字典维护 `scan_phase`、`scan_log`、`scan_kind`、`scan_result`
- `start_v4_market_scan()` — 后台线程执行 V4 扫描，避免 Web 请求超时
- `append_scan_log()` — 逐行解析扫描日志，自动更新阶段
- `/api/scan/status` — 轮询接口，返回完整状态 + 日志
- `/api/v4/market-scan` — 启动 V4 扫描任务，不再同步返回完整扫描结果
...
- 支持最小化到左下角气泡，点击气泡可恢复弹窗
- HTML/JS/CSS 资源返回 no-cache，并通过 `?v=...` 版本号降低线上旧脚本缓存风险
- 完成后变为 100% 进度条
...
| v3.1 | 2026-05-13 | 实时行情切换腾讯直连；候选股收益展示修复；扫描弹窗最小化 |
| v4.0 | 2026-05-14 | 尾盘策略闭环工作台：风控过滤、候选池跟踪、次日验证、V4 CLI + Web UI |
| v4.1 | 2026-05-14 | 自动定时流程、决策解释、真实分钟K早盘验证、风控剔除视图 |
| v4.2 | 2026-05-14 | V4 扫描接入异步进度弹窗；HTML/JS/CSS 禁止缓存并增加资源版本号 |
